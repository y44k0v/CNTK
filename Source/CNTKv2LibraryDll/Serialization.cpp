...
namespace CNTK
{
    ...
    {
    public:
        InputWrapper(io::ZeroCopyInputStream& input) 
            : m_input(input)
        {
            Refresh();
        }
    ...
    private:
        ...
        template <typename T>
        static bool ReadData(InputWrapper& input, NDArrayView& dst)
        {
    ...
    void Serializer::Write(io::ZeroCopyOutputStream& stream) {
        ...
        {
            m_proto->SerializeToCodedStream(&output);
            // This 0 tag serves as a sentinel, it signals an end of message to 
            // the protobuf parser.
            output.WriteTag(0);
    ...
    bool ParseMessage(io::ZeroCopyInputStream& input, Message& msg)
    {
        ...
        return msg.ParseFromCodedStream(&codedInput) &&
            (codedInput.LastTagWas(0) || codedInput.ConsumedEntireMessage());
    }
    ...
    {
        io::IstreamInputStream input(&stream);
